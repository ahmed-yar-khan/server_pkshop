...
mongoose.connect(
  "mongodb+srv://ahmeddehwar2:PKshop@pkshop.tvb81o0.mongodb.net/pkshop",
  {
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, "E:/pkshop/server/uploaded");
...
app.post(
  "/api/products",
  upload.array("additionalImages", 5),
  async (req, res) => {
    try {
      ...
      // Parse weightOptions
      const weightOptions = productData.weightOptions.map((item) =>
        JSON.parse(item)
      );

      // Create a new Product with the parsed data
      const newProduct = new Product(productData);

      // Assign the weightOptions
      newProduct.weightOptions = weightOptions;
      const originalFileNames = Array.isArray(req.files)
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      productData.additionalImages = additionalImagesData;
      productData.weightOptions = weightOptions;
...
app.get("/api/orders", async (req, res) => {
  try {
    ...
    if (value && value.trim() !== "") {
      filter.$or = [];
      filter.$or.push({ firstname: { $regex: new RegExp(value, "i") } });
      filter.$or.push({ email: { $regex: new RegExp(value, "i") } });
    }
...
app.post("/check-session", function (req, res) {
  ...
  jwt.verify(
    token,
    "shoe store wali user ki session",
...
app.use("/uploaded", express.static("./data"));
